...
class RepositoryListViewController: UITableViewController {
    ...
    private let reposigories = BehaviorSubject<[Repository]>(value: [])
    ...
    func fetchRepositories(of organization: String) {
        Observable.from([organization])
            .map { origanization -> URL in
                return URL(string: "https://api.github.com/orgs/\(origanization)/repos")!
            }
            ...
            .subscribe(onNext: { [weak self] newRepositories in
                self?.reposigories.onNext(newRepositories)
...
extension RepositoryListViewController {
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        do {
            return try reposigories.value().count
        } catch {
            return 0
        }
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var currentRepo: Repository? {
            do {
                return try reposigories.value()[indexPath.row]
            } catch {
                return nil
            }
        }
